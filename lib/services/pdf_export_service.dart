...
class PdfExportService {
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Row(
              ...
              children: [
                pw.Text('ЗАТВЕРДЖЕНО МОЗ УКРАЇНИ\nПротокол надання реабілітаційної допомоги', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                pw.Text('Форма первинної фіксації ІРП', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
              ],
            ),
            ...
            pw.Center(
              child: pw.Text('Етап: Первинне призначення та стратегія відновлення', style: const pw.TextStyle(fontSize: 11, color: PdfColors.grey900)),
            ),
            ...
            if (assessments.isEmpty)
              pw.Paragraph(text: 'Інтерактивні об\'єктивні шкали на момент формування плану не проводилися. Оцінка проведена на основі загального соматичного статусу.', style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey800))
            else
              pw.Column(
                  ...
                  return pw.Padding(
                    ...
                    child: pw.Row(
                      ...
                      children: [
                        ...
                        pw.SizedBox(width: 6),
                        pw.Expanded(
                          child: pw.Text('${res.scaleName}: ${res.interpretation} (Дата: ${res.date.day}.${res.date.month}.${res.date.year})', style: const pw.TextStyle(fontSize: 10)),
                        ),
                      ],
                    ),
                  );
            ...
            pw.Container(
              ...
              child: pw.Text(irpPlan.smartGoals, style: const pw.TextStyle(fontSize: 10, height: 1.3)),
            ),
            pw.SizedBox(height: 12),
            ...
            pw.Row(
              ...
              children: [
                pw.Column(
                  cross pw.CrossAxisAlignment.start,
                  children: [
                    pw.Container(width: 120, border: const pw.Border(bottom: pw.BorderSide(width: 1, color: PdfColors.black))),
                    pw.SizedBox(height: 4),
                    pw.Text('Підпис фізичного терапевта / лікаря', style: const pw.TextStyle(fontSize: 8)),
                  ],
                ),
                pw.Column(
                  cross pw.CrossAxisAlignment.start,
                  children: [
                    pw.Container(width: 120, border: const pw.Border(bottom: pw.BorderSide(width: 1, color: PdfColors.black))),
                    pw.SizedBox(height: 4),
                    pw.Text('З планом ознайомлений (пацієнт)', style: const pw.TextStyle(fontSize: 8)),
                  ],
                ),
              ],
            ),
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Row(
              main pw.MainAxisAlignment.spaceBetween,
              children: [
                pw.Text('МЕДИЧНА ДОКУМЕНТАЦІЯ ЗА СТАНДАРТАМИ НСЗУ\nКод форми за ДКУД', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                pw.Text('Витяг з медичної карти / Виписка з ІРП', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
              ],
            ),
            pw.SizedBox(height: 16),
            ...
            if (assessments.isEmpty)
              pw.Paragraph(text: 'Для даного пацієнта не зафіксовано повторних або первинних тестувань у базі даних додатку.', style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey700))
            else
              pw.Table(
                ...
                children: [
                  ...
                  ...assessments.map((res) {
                    return pw.TableRow(
                      children: [
                        pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(res.scaleName, style: const pw.TextStyle(fontSize: 9))),
                        pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(res.interpretation, style: const pw.TextStyle(fontSize: 9))),
                      ],
                    );
                  }).toList(),
                ],
            ...
            pw.Bullet(text: 'Продовжити щоденне виконання індивідуального комплексу ЛФК у домашніх умовах.', style: const pw.TextStyle(fontSize: 9)),
            pw.Bullet(text: 'Дотримуватись ортопедичного або неврологічного режиму безпеки руху (контроль пози тіла, уникнення перевантажень).', style: const pw.TextStyle(fontSize: 9)),
            pw.Bullet(text: 'Повторний огляд мультидисциплінарною реабілітаційною командою (МРК) через 3 місяці.', style: const pw.TextStyle(fontSize: 9)),
            ...
            pw.Row(
              main pw.MainAxisAlignment.spaceBetween,
              children: [
                pw.Text('Дата виписки: ${DateTime.now().day}.${DateTime.now().month}.${DateTime.now().year} р.', style: const pw.TextStyle(fontSize: 10)),
                pw.Column(
                  cross pw.CrossAxisAlignment.end,
                  children: [
                    pw.Container(width: 150, border: const pw.Border(bottom: pw.BorderSide(width: 1, color: PdfColors.black))),
                    pw.SizedBox(height: 4),
                    pw.Text('Підпис голови МРК / Фізичного терапевта', style: const pw.TextStyle(fontSize: 8)),
                  ],
                ),
              ],
            ),
  ...
  static pw.Widget _buildPdfSectionTitle(String title) {
    return pw.Padding(
      ...
      child: pw.Text(
        title,
        style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColors.blueDark),
      ),
    );
  }

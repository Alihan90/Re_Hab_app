...
/// Модель згенерованого індивідуального реабілітаційного плану
class SmartPlan {
  final String goalsSmart;
  final String mfkCodes; // Міжнародна класифікація функціонування
...
/// Модель елемента фізичної чи апаратної терапії
class ExerciseItem {
  final String title;
  final String category; // Кінезотерапія, Механотерапія, Електростимуляція, Магнітотерапія, Інвентар
  final String dosage;

  ExerciseItem({required this.title, required this.category, required this.dosage});
}

/// Модель клінічної шкали для каталогу
class ClinicalScale {
  final String id;
  final String name;
  final String fullName;
  final String category; // Неврологія, Педіатрія, Ортопедія, Загальні
  final String ageGroup; // Дорослі, Діти, Всі
...
class SmartIrpEngine {
  /// 1. ПОВНИЙ ДОВІДНИК КЛІНІЧНИХ ШКАЛ за нозологіями та віком (42 шкали)
  final List<ClinicalScale> scalesCatalog = [
    ...
    ClinicalScale(id: 'moca', name: 'MoCA', fullName: 'Монреальська шкала когнітивної оцінки', category: 'Неврологія', ageGroup: 'Дорослі', minScore: 0, maxScore: 30, interpretation: '26-30: норма; менше 26: когнітивні порушення/деменція'),
    ...
    // --- ЗАГАЛЬНІ, КАРДІО-РЕСПІРАТОРНІ ТА ХОДЬБА ---
  ...
  /// 2. КЛІНІЧНИЙ ДОВІДНИК МКХ-10 ДЛЯ РЕАБІЛІТАЦІЇ
  ...
  SmartPlan autoGeneratePlan({
    ...
    // Перевірка на ПРОТЕЗУВАННЯ / АМПУТАЦІЮ ПЕРЕДПЛІЧЧЯ (Z89.2)
    if (selectedCode.toUpperCase().contains('Z89.2')) {
      ...
      baseExercises = [
        ...
        ExerciseItem(title: 'БОЗ-активація (Біозворотний зв\'язок) флексорів та екстензорів для керування міо-протезом', category: 'Тренажерні методи', dosage: '15 хв, 30 повторень'),
        ExerciseItem(title: 'Електростимуляція (СМТ/ТЕНС) цільових м\'язових груп кукси лівого передпліччя', category: 'Електростимуляція', dosage: '12 хв, струм до 15 мА'),
        ExerciseItem(title: 'Низькочастотна магнітотерапія на область рубця передпліччя для покращення трофіки', category: 'Магнітотерапія', dosage: '15 хв, індукція 20 мТл'),
        ExerciseItem(title: 'Ерготерапевтичне тренування дворучного хвату (маніпуляції з кубиками, циліндрами)', category: 'З інвентарем', dosage: '25 хв, до втоми'),
      ];
    }
    // Перевірка на ІНСУЛЬТ (I63.9)
      ...
      baseExercises = [
        ...
        ExerciseItem(title: 'Функціональна електростимуляція (ФЕС) розгиначів кисті та стопи під час кроку', category: 'Електростимуляція', dosage: '15 хв, частота 40 Гц'),
        ExerciseItem(title: 'Заняття на стабілоплатформі з візуальним контролем для відновлення симетрії опори', category: 'Тренажерні методи', dosage: '15 хв, 3 серії'),

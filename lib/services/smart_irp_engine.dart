  ...
  static SmartIrpPlan generate({
    ...
    if (hasMultiorganFailure) {
      goals = "S (Специфічна): Профілактика гіпостатичних ускладнень та іммобілізаційного синдрому.\n"
              "M (Вимірювана): Збереження об'єму пасивних рухів у великих суглобах на початковому рівні (кут у градусах).\n"
      ...
      goals = "S: Підвищення толерантності до фізичних навантажень та покращення дренажної функції легень.\n"
              "M: Збільшення дистанції у Тесті 6-хвилинної ходьби на 50 м, зниження задишки за шкалою mMRC на 1 бал.\n"
    ...
    for (int day = 1; day <= treatmentDays; day++) {
      ...
      if (hasMultiorganFailure) {
        if (day <= phase1End) {
          dailyDescription = "Фаза I: Щадна пасивна. Позиційне лікування (антидекубітальний протокол) кожні 2 години. Пасивні рухи у дистальних суглобах кінцівок (10-15 повторень). Легкий дренажний масаж грудної клітки. Постійний моніторинг реанімаційних показників.";
        } else {
          dailyDescription = "Фаза II: Стабілізаційна. Дихальні вправи (пасивне спонукання до глибокого видиху). Пасивна мобілізація всіх великих суглобів для запобігання тугорухливості. Ліжкова вертикалізація головного кінця ліжка до 30-45 градусів (контроль АТ!).";
        }
      } else if (isNeuro) {
        if (day <= phase1End) {
          ...
          dailyDescription = "Фаза III: Функціональна. Тренування ходи в брусах або з ходунками. Вправи на координацію та баланс (вправи стоячи, перенесення ваги тіла). Ерготерапія: відновлення навичок самообслуговування, застібання ґудзиків, тримання ложки. Ергономіка ходи.";
        }
      } else if (isOrtho) {
        if (day <= phase1End) {
          dailyDescription = "Фаза I: Протибольова та протинабрякова. Ізометрична гімнастика для м'язів навколо оперованого/травмованого суглоба. Пасивні рухи на Kinetec (CPM-терапія) у межах безболісної амплітуди. Кріотерапія локально (10-15 хв) після занять для зменшення набряку. Навчання правилам безпечного переміщення.";
        } else if (day <= phase2End) {
          dailyDescription = "Фаза II: Відновлення амплітуди рухів (ROM). Активні вправи з розвантаженням кінцівки (блокові системи, підвісна терапія). М'які мануальні техніки мобілізації суглоба. Стретчинг спазмованих м'язових груп. Тренування ходи на милицях із чітким дозованим навантаженням на стопу (% від ваги тіла).";
        } else {
          dailyDescription = "Фаза III: Силова та пропріоцептивна. Вправи з легким опором (еластичні стрічки Thera-Band). Тренування балансу на балансувальних подушках для відновлення суглобового чуття (пропріоцепції). Корекція стереотипу ходи без допоміжних засобів (або з тростиною). Клінічний гоніометричний контроль.";
        }
      } else {
        if (day <= phase1End) {
